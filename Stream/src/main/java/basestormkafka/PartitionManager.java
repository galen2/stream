...
import basestormkafka.KafkaSpouts.EmitState;
import basestormkafka.KafkaSpouts.MessageAndRealOffset;
import basestormkafka.trient.MaxMetric;
...
public class PartitionManager {
    ...
    private final CountMetric _fetchAPIMessageCount;//从kafka读取了多少条数据指标
        ...
        while (true) {
            ...
            if (tups != null) {
                for (List<Object> tup : tups) {
                	//获取到消息之后，传递到下一个bolt进行处理
                	collector.emit(tup, new KafkaMessageId(_partition, toEmit.offset));
                	Object object = tup.get(0);
    ...
    /**
     * 从kafka消费“一批”数据，然后把数据封装成MessageAndRealOffset，最终存放到_waitingToEmit集合里面
     */
    private void fill() {
        ...
        if (msgs != null) {
            ...
            for (MessageAndOffset msg : msgs) {
                ...
                if (processingNewTuples || this._failedMsgRetryManager.shouldRetryMsg(cur_offset)) {
                    ...
                    //把msg分装成MessageAndRealOffset存放到_waitingToEmit集合里面
